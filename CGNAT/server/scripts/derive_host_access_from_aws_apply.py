...
def _instance_record(apply_result: dict[str, Any], role_key: str) -> dict[str, Any]:
    return apply_result[role_key].get("response", {}).get("Instances", [{}])[0]
...
def _resolve_target_host(apply_result: dict[str, Any], strategy: dict[str, Any], service_role: str) -> str:
    role_key = "head_end" if service_role == "cgnat_head_end" else "isp_head_end"
    instance = _instance_record(apply_result, role_key)
