...
def derive_passthrough_dataplane(
    ...
    # Match the MUXER3 pass-through contract: strict non-NAT keeps the shared
    # public identity as the delivery destination, while NAT-T is delivered to
    # the backend underlay IP.
    nat_preroute_dst = str((transport["backend_underlay_ip"] if udp4500 else public_ip) or "").strip()
