...
def _render_master_remove_script(layout: dict[str, Path], customer_name: str) -> str:
    ...
    return _render_shell_script(
        [
            ...
            'CUST="${CUSTOMER_ROOT##*/}"',
            f'SWANCTL_CONF="${{ROOT}}{swanctl_conf}"',
            'SWANCTL_FLAT_CONF="${ROOT}/etc/swanctl/conf.d/${CUST}.conf"',
            'IPSEC_CUSTOMER_CONF="${ROOT}/etc/ipsec.d/customers/${CUST}.conf"',
            'rm -f "${SWANCTL_CONF}"',
            ...
            "  done",
            "fi",
            'for unit_path in "${ROOT}"/etc/systemd/system/${CUST}-*.service "${ROOT}"/etc/systemd/system/multi-user.target.wants/${CUST}-*.service; do',
            '  [ -e "${unit_path}" ] || continue',
            '  rm -f "${unit_path}"',
            "done",
            'for script_path in "${ROOT}"/usr/local/sbin/${CUST}-*.sh; do',
