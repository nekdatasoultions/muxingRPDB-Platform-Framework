...
def muxer_remove_command(customer_name: str) -> str:
    ...
    return "\n".join(
        [
            ...
            'MODULE_ROOT="/etc/muxer/config/customer-modules/${CUST}"',
            'LEGACY_MODULE_ROOT="/etc/muxer/customer-modules/${CUST}"',
            ...
            'elif [ -f "${LEGACY_MODULE_ROOT}/customer-module.json" ] && [ -f /etc/muxer/src/muxctl.py ]; then',
            '  python3 /etc/muxer/src/muxctl.py remove-customer "${CUST}"',
            "fi",
            'nft delete table ip "${MX_TABLE}" 2>/dev/null || true',
            'rm -rf "${CUSTOMER_ROOT}" "${MODULE_ROOT}" "${LEGACY_MODULE_ROOT}"',
            'test ! -e "${CUSTOMER_ROOT}"',
            'test ! -e "${MODULE_ROOT}"',
            'test ! -e "${LEGACY_MODULE_ROOT}"',
...
def headend_remove_command(customer_name: str) -> str:
    ...
    return "\n".join(
        [
            ...
            'SWANCTL_FLAT_CONF="/etc/swanctl/conf.d/${CUST}.conf"',
            'IPSEC_CUSTOMER_CONF="/etc/ipsec.d/customers/${CUST}.conf"',
            'HEADEND_CERT="/etc/swanctl/x509/rpdb-customers/${CUST}-headend-cert.pem"',
            'HEADEND_KEY="/etc/swanctl/private/${CUST}-headend-key.pem"',
            'HEADEND_KEY_OLD="/etc/swanctl/private/rpdb-customers/${CUST}-headend-key.pem"',
            'REMOTE_TRUST="/etc/swanctl/x509ca/rpdb-customers/${CUST}-remote-trust.pem"',
            'REMOTE_CERT="/etc/swanctl/x509/rpdb-customers/${CUST}-remote-cert.pem"',
            ...
            '  for unit_path in /etc/systemd/system/${CUST}-*.service /etc/systemd/system/multi-user.target.wants/${CUST}-*.service; do',
            '    [ -e "${unit_path}" ] || continue',
            '    unit_name="$(basename "${unit_path}")"',
            '    systemctl disable --now "${unit_name}" 2>/dev/null || systemctl stop "${unit_name}" 2>/dev/null || true',
            "  done",
            "fi",
            'for unit_path in /etc/systemd/system/${CUST}-*.service /etc/systemd/system/multi-user.target.wants/${CUST}-*.service; do',
            '  [ -e "${unit_path}" ] || continue',
            '  rm -f "${unit_path}"',
            "done",
            'for script_path in /usr/local/sbin/${CUST}-*.sh; do',
            ...
            'rm -f "${SWANCTL_FLAT_CONF}"',
            'rm -f "${IPSEC_CUSTOMER_CONF}"',
            'rm -f "${HEADEND_CERT}" "${HEADEND_KEY}" "${HEADEND_KEY_OLD}" "${REMOTE_TRUST}" "${REMOTE_CERT}"',
            'nft delete table ip "${HN_TABLE}" 2>/dev/null || true',
            'nft delete table ip "${ON_TABLE}" 2>/dev/null || true',
            'if command -v swanctl >/dev/null 2>&1 && systemctl is-active --quiet strongswan; then',
            "  swanctl --load-all",
            "  for _ in $(seq 1 30); do",
            '    if ! swanctl --list-sas 2>/dev/null | grep -q "${CUST}"; then',
            "      break",
            "    fi",
            "    sleep 1",
            "  done",
            '  if swanctl --list-sas 2>/dev/null | grep -q "${CUST}"; then',
            '    echo "head-end strongSwan SA still exists for ${CUST}"',
            "    exit 43",
            "  fi",
            "fi",
            'test ! -e "${CUSTOMER_ROOT}"',
            'test ! -e "${SWANCTL_CONF}"',
            'test ! -e "${SWANCTL_FLAT_CONF}"',
            'test ! -e "${IPSEC_CUSTOMER_CONF}"',
            'test ! -e "${HEADEND_CERT}"',
            'test ! -e "${HEADEND_KEY}"',
            'test ! -e "${HEADEND_KEY_OLD}"',
            'test ! -e "${REMOTE_TRUST}"',
            'test ! -e "${REMOTE_CERT}"',
