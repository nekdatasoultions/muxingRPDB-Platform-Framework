...
CGNAT_SRC = REPO_ROOT / "CGNAT" / "framework" / "src"
if str(CGNAT_SRC) not in sys.path:
    sys.path.insert(0, str(CGNAT_SRC))

from muxerlib.customer_merge import load_yaml_file
from cgnat.customer_provisioning import validate_cgnat_request
from cgnat.pki_materializer import materialize_cgnat_pki
...
def _build_cgnat_deployment_review(
    *,
    customer_name: str,
    customer_file: Path,
    target_selection: dict[str, Any],
) -> dict[str, Any]:
    review_root = REPO_ROOT / "CGNAT" / "build" / "customer-deploy" / customer_name
    pki_root = review_root / "pki"
    if review_root.exists():
        shutil.rmtree(review_root)
    review_root.mkdir(parents=True, exist_ok=True)

    request_doc = load_yaml_file(customer_file)
    validate_cgnat_request(request_doc, request_path=_repo_relative(customer_file))
    pki_review = materialize_cgnat_pki(request_doc, pki_root)
    pki_review_path = pki_root / "pki-review.json"
    _write_json(pki_review_path, pki_review)

    status = "ready_for_review" if bool(pki_review.get("ready_for_review")) else "blocked"
    review = {
        "schema_version": 1,
        "surface": "deploy_customer_cgnat",
        "status": status,
        "ready_for_review": status == "ready_for_review",
        "generated_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "customer_name": customer_name,
        "transport": {
            "outer_topology": target_selection.get("cgnat_outer_topology"),
            "outer_gateway_ref": target_selection.get("cgnat_outer_gateway_ref"),
            "headend_target": ((target_selection.get("cgnat_headend_active") or {}).get("name")),
            "gateway_target": ((target_selection.get("cgnat_isp_gateway") or {}).get("name")),
        },
        "pki": {
            "mode": pki_review.get("mode"),
            "provider": pki_review.get("provider"),
            "status": pki_review.get("status"),
            "ready_for_review": pki_review.get("ready_for_review"),
            "generated_material": pki_review.get("generated_material"),
            "provided_material": bool((pki_review.get("artifacts") or {}).get("provided_material")),
            ...
            "artifacts": pki_review.get("artifacts"),
        },
        "paths": {
            "review_root": _repo_relative(review_root),
            "pki_review_dir": _repo_relative(pki_root),
            "pki_review": _repo_relative(pki_review_path),
        },
        "notes": [
            "This review was generated by scripts/customers/deploy_customer.py for a CGNAT customer.",
            "Live apply consumes the PKI review path when preparing the CGNAT head-end payload.",
        ],
    }
    _write_json(review_root / "cgnat-deployment-review.json", review)
    return review
...
def _build_execution_plan(
    *,
    ...
    cgnat_review: dict[str, Any] | None = None,
) -> dict[str, Any]:
    readiness = (package_report or {}).get("readiness") or {}
    ...
    return {
        ...
        "execution_order": [
            ...
            *(["prepare_cgnat_deployment_review"] if cgnat_review is not None else []),
        ...
        "artifacts": {
            ...
            "cgnat_review": (
                ((cgnat_review or {}).get("paths") or {}).get("review_root")
                if cgnat_review
                else None
            ),
            "cgnat_pki_review": (
                ((cgnat_review or {}).get("paths") or {}).get("pki_review")
                if cgnat_review
                else None
            ),
        },
        ...
        if not errors:
            ...
            if package_code != 0 or not package_report or package_report.get("status") != "ready_for_review":
                ...
                if not errors:
                    ...
                    if bool((target_selection or {}).get("cgnat_required")):
                        try:
                            cgnat_review = _build_cgnat_deployment_review(
                                customer_name=customer_name,
                                customer_file=customer_file,
                                target_selection=target_selection or {},
                            )
                            if not cgnat_review.get("ready_for_review"):
                                errors.append("CGNAT deployment review is not ready")
                        except Exception as exc:
                            errors.append(f"CGNAT deployment review failed: {exc}")
